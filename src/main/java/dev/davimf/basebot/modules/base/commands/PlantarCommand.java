...
/** /economia plantar — colheita; exige equipamento de fazenda equipado. */
public final class PlantarCommand implements SlashCommand {
    private final JobService jobs;
    private final JailService jail;
    public PlantarCommand(JobService jobs, JailService jail) { this.jobs = jobs; this.jail = jail; }

    @Override public String name() { return "plantar"; }
    @Override public SlashCommandData data() { return Commands.slash("plantar", "Planta e colhe com o equipamento de fazenda equipado."); }
    ...
    @Override public void execute(SlashCommandInteractionEvent event, BotContext ctx) {
        ...
        Replies.reply(event, ctx, jobs.runPlant(event.getGuild(), event.getMember()));
    }
}
