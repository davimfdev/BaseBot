...
/** /economia faturar — recolhe lucros; exige contrato/licença equipado. */
public final class FaturarCommand implements SlashCommand {
    private final JobService jobs;
    private final JailService jail;
    public FaturarCommand(JobService jobs, JailService jail) { this.jobs = jobs; this.jail = jail; }

    @Override public String name() { return "faturar"; }
    @Override public SlashCommandData data() { return Commands.slash("faturar", "Fatura os lucros do seu negócio com o contrato equipado."); }
    ...
    @Override public void execute(SlashCommandInteractionEvent event, BotContext ctx) {
        ...
        Replies.reply(event, ctx, jobs.runInvoice(event.getGuild(), event.getMember()));
    }
}
