...
public final class FormularioCommand implements SlashCommand {
    ...
    private void criar(SlashCommandInteractionEvent event) {
        ...
        event.reply("📋 Formulário criado (`" + questions.size() + "` perguntas).\n"
                + "Publique com `/formulario enviar id:" + id + "`").setEphemeral(true).queue();
    }
    ...
    private void enviar(SlashCommandInteractionEvent event, BotContext ctx) {
        ...
        if (form.isEmpty() || !event.getGuild().getId().equals(form.get().guildId())) {
            event.reply("Formulário não encontrado.").setEphemeral(true).queue();
            return;
        }
    ...
    private void lista(SlashCommandInteractionEvent event) {
        ...
        if (all.isEmpty()) {
            event.reply("Nenhum formulário cadastrado.").setEphemeral(true).queue();
            return;
        }
        StringBuilder sb = new StringBuilder("## 📋 Formulários\n");
        for (Form f : all) {
