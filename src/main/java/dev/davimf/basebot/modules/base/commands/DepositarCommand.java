...
/** /depositar — carteira → banco (sem valor = tudo). */
public final class DepositarCommand implements SlashCommand {
    private final EconomyService eco;
    public DepositarCommand(EconomyService eco) { this.eco = eco; }

    @Override public String name() { return "depositar"; }

    @Override public SlashCommandData data() {
        return Commands.slash("depositar", "Deposita moedas da carteira no banco.")
                .addOption(OptionType.INTEGER, "quantia", "Quanto depositar (vazio = tudo)", false);
    }
