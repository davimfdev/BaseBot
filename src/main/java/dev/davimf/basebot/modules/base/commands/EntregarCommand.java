...
/** /entregar — emprego que exige moto equipada; consome combustível da carteira. */
public final class EntregarCommand implements SlashCommand {
    private final JobService jobs;
    private final JailService jail;
    public EntregarCommand(JobService jobs, JailService jail) { this.jobs = jobs; this.jail = jail; }

    @Override public String name() { return "entregar"; }
    @Override public SlashCommandData data() { return Commands.slash("entregar", "Faz uma entrega com a moto equipada."); }
    ...
    @Override public void execute(SlashCommandInteractionEvent event, BotContext ctx) {
        ...
        Replies.reply(event, ctx, jobs.runDelivery(event.getGuild(), event.getMember()));
    }
}
