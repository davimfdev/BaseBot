...
/** /cozinhar — emprego que exige utensílio de cozinha equipado. */
public final class CozinharCommand implements SlashCommand {
    private final JobService jobs;
    private final JailService jail;
    public CozinharCommand(JobService jobs, JailService jail) { this.jobs = jobs; this.jail = jail; }

    @Override public String name() { return "cozinhar"; }
    @Override public SlashCommandData data() { return Commands.slash("cozinhar", "Cozinha com o utensílio equipado."); }
    ...
    @Override public void execute(SlashCommandInteractionEvent event, BotContext ctx) {
        ...
        Replies.reply(event, ctx, jobs.runCooking(event.getGuild(), event.getMember()));
    }
}
